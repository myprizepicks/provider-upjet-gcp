...
func Configure(p *config.Provider) { //nolint: gocyclo
	...
	p.AddResourceConfigurator("google_compute_network_firewall_policy_association", func(r *config.Resource) {
		...
		r.TerraformResource.Read = func(d *schema.ResourceData, meta interface{}) error {
			...
			if err != nil && transport_tpg.IsGoogleApiErrorWithCode(err, 400) {
				log.Printf("[WARN] Removing NetworkFirewallPolicyAssociation %q because GCP returned 400 (treating as not found)", d.Id())
				d.SetId("")
				return nil
			}
			return err
	...
	p.AddResourceConfigurator("google_compute_region_network_firewall_policy_association", func(r *config.Resource) {
		...
		r.TerraformResource.Read = func(d *schema.ResourceData, meta interface{}) error {
			err := originalRead(d, meta)
			if err != nil && transport_tpg.IsGoogleApiErrorWithCode(err, 400) {
				log.Printf("[WARN] Removing RegionNetworkFirewallPolicyAssociation %q because GCP returned 400 (treating as not found)", d.Id())
				d.SetId("")
				return nil
			}
			return err
